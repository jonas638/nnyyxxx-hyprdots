...
symlink_path="$HOME/wallpapers/selected.jpg"
last_value=""

inotifywait -m -e modify,create,attrib,move,moved_to "$(dirname "$symlink_path")" | while read -r directory events filename; do
    if [ "$filename" = "$(basename "$symlink_path")" ]; then
        current_value=$(readlink -f "$symlink_path")
...
cat > "$HOME/.config/rofi/themes/pywal.rasi" <<EOF
* {
  background: #${color0};
  foreground: #${color7};
  border:     #${color2};
